...
namespace RocketFooxball.Editor
{
    internal static partial class MovementLabLightingPipeline
    {
                ...
                {
                    BakeSceneLighting(scene, passPath, MovementLabLightingProfiles.ProfileId.Production);
                }

                internal static void BakeSceneLighting(Scene scene, string passPath, MovementLabLightingProfiles.ProfileId profile)
                {
                    // Profile selection and validation happen before entering
                    // Unity's bake boundary. No setting/material repair is
                    // permitted after Lightmapping.Bake starts.
                    MovementLabLightingProfiles.LoadValidatedSettings(profile);
                    MovementLabLightingProfiles.PrepareScene(scene, profile);
                    MovementLabLightingProfiles.ValidatePreparedScene(profile);
                    if (!string.IsNullOrWhiteSpace(passPath)) MovementLabPreBakeGate.RevalidatePassRecord(passPath);
