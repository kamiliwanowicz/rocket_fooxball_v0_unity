...
{
    /// <summary>
    /// Shared generated-path, serialized-helper, and stage bridge context.
    /// Domain methods live in MovementLab pipeline modules below.
    /// </summary>
    internal static partial class MovementLabBuildContext
    {
        ...
        internal static string[] CreateGeneratedFingerprintPaths()
        {
        ...
        internal static void AddGeneratedFingerprintPath(List<string> paths, HashSet<string> seen, string path)
        {
        ...
        internal static void ValidateGeneratedFingerprintPathList(string[] paths)
        {
        ...
        internal static string ComputeBuilderSignature()
        {
            return "serialized-contract-v" + MovementLabContract.SerializedContractVersion;
        }

        internal static string GetBuildMarkerName(string builderSignature)
        {
            return BuildMarkerPrefix + builderSignature;
        }

        // Shared context bridges. Domain methods live in pipeline classes.
        internal static void AssembleGeneratedAssets() => MovementLabSceneComposer.AssembleMovementLabUnstaged();

        internal static void ValidateGeneratedAssets(bool includeBakedLighting, bool logSuccess)
        {
            MovementLabValidator.ValidateMovementLabInternal(ComputeBuilderSignature(), includeBakedLighting, logSuccess);
        }

        internal static void ConfigureImporters()
        {
            MovementLabImportPipeline.ConfigureTextureImporters();
            MovementLabImportPipeline.ConfigureModelImporters();
        }

        internal static void ValidateImporterContracts()
        {
            MovementLabImportPipeline.ValidateTextureImporterContracts();
            MovementLabImportPipeline.ValidateModelImporterContracts();
        }

        internal static void FinalizeMaterialPersistence() => MovementLabMaterialPipeline.FinalizeGeneratedMaterialPersistence();

        internal static void BakeLighting(Scene scene, string passPath) => MovementLabLightingPipeline.BakeSceneLighting(scene, passPath);

        internal static void NormalizeGeneratedAssets() => MovementLabSceneComposer.NormalizeGeneratedYamlWhitespace();

        internal static string SceneAssetPath => ScenePath;
        internal static string ManifestAssetPath => ManifestPath;
    }

    /// <summary>
    /// Stable Unity menu facade. Commands resolve stage state, dispatch one
    /// domain owner, and report outcomes. Generation and validation live in
    /// MovementLab pipeline modules.
    /// </summary>
    public static class MovementLabBuilder
    {
        ...
        public static void AssembleMovementLab()
        {
            ...
            {
                Debug.Log("Rocket Fooxball Movement Lab assembly reused generated state: " + MovementLabBuildContext.SceneAssetPath);
                return;
            }

            MovementLabManifestStore.EnsureWriteAuthorization();
            MovementLabSceneComposer.Assemble();
            MovementLabManifestStore.WriteAtomic(MovementLabStageGraph.CaptureAssembledState());
            AssetDatabase.ImportAsset(MovementLabBuildContext.ManifestAssetPath, ImportAssetOptions.ForceSynchronousImport);
            Debug.Log("Rocket Fooxball Movement Lab assembled without lighting bake: " + MovementLabBuildContext.SceneAssetPath);
        }
        ...
        {
            var passPath = MovementLabPreBakeGate.ValidateAndWritePassRecord();
            var scene = UnityEditor.SceneManagement.EditorSceneManager.OpenScene(MovementLabBuildContext.SceneAssetPath, UnityEditor.SceneManagement.OpenSceneMode.Single);
            MovementLabPreBakeGate.RevalidatePassRecord(passPath);
            MovementLabLightingPipeline.Bake(scene, passPath);
            UnityEditor.SceneManagement.EditorSceneManager.SaveScene(scene, MovementLabBuildContext.SceneAssetPath);
            AssetDatabase.SaveAssets();
            MovementLabBuildContext.NormalizeGeneratedAssets();
            AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport);
            MovementLabManifestStore.WriteAtomic(MovementLabStageGraph.CaptureBakedState());
            AssetDatabase.ImportAsset(MovementLabBuildContext.ManifestAssetPath, ImportAssetOptions.ForceSynchronousImport);
            Debug.Log("Rocket Fooxball Movement Lab lighting baked explicitly: " + MovementLabBuildContext.SceneAssetPath);
        }
