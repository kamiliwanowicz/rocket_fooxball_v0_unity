...
- implementation: jump phase -> reset match; wait grounded up to `30` fixed steps; record horizontal speed; hold jump; require positive vertical velocity and a horizontal gain greater than upward launch speed; keep held across `6` fixed steps and prove no second upward discontinuity; release one fixed step; press again while airborne and prove no upward discontinuity; release; complete four bounded grounded jump cycles and require `HorizontalSpeed >= HardCap - 0.05`; assert containment
