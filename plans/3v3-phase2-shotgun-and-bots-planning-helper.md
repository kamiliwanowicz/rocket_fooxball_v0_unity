...
- I3 Whole-scene renderer budget is 140 (`MovementLabLightingPipeline.cs:481-513`, counts inactive children). Current usage ~91. Shotgun viewmodel is 3 renderers x 6 participant instances = +18; a world/right-hand model adds up to +18 more; 3 new pickup instances at 3 renderers each = +9 -> ~136 of 140. This is the tightest hard limit in the run and it fails late, inside lighting/pre-bake validation, after import + prefab + scene writes have landed. Budget the shotgun renderer count before authoring C4; consider merging the three material slots into fewer renderers, or gating the world model.
