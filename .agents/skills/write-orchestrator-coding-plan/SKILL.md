...
- Resolve concrete findings. Each audit role gets initial audit plus at most two targeted fresh re-audits. Each re-audit receives revised draft plus prior finding evidence and reports only surviving or new evidence-backed findings. Second targeted re-audit is closure-only: dispatch only when first re-audit exposes concrete, locally resolvable gap; restrict scope to applied correction and affected proof seams. Gap surviving or newly exposed after second targeted re-audit -> `needs_user`, `blocked`, or decomposition mismatch; never continue audit loop. Accept only when every role reports `Worker Decisions Remaining: None` and `Gaps: None`. Do not add process or abstraction without identified failure it prevents.
