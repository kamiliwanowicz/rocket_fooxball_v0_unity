...
Every changed authoritative output, owned or inventory-exception, requires exact-SHA coverage at checkpoint/final: changed path/source/scope; selected/output sets; matching headers; `SEMANTIC:`, `DANGLING:`, `GUID:`, `PAIRS:`, `UNSUPPORTED:`. Run `Tools/Validation/Compare-GeneratedYaml.ps1 -Base <start_sha> -Head <plan_head> -FailOnDangling`. Builder-traced task-declared semantic changes are valid. Binary hashes bind provenance only. Separate regeneration commit or excluded raw slice never removes semantic-evidence review. Missing/stale/incomplete coverage, unknown type, comparator failure, dangling increase, existing-GUID churn, broken pairing -> reject.
