...
- accepted `integration_fixer.Status: complete`: close only integration-fix child task/attempt/lease; set linked findings `fixed | rejected` with disposition evidence; set `final_stage_fix_cycle_used: 1`; retain substate `integration_fix`. Child `Final frozen head: pending_plan_supervisor_freeze` records no SHA and writes no gate or integration-head field
- accepted `integration_fixer.Status: blocked`: close only integration-fix child task/attempt/lease; retain open finding and blocker; set `final_stage_fix_cycle_used: 1`; retain substate `integration_fix`. Child acceptance writes no gate or integration-head field
- parent integration-fix freeze: after accepted complete fixer result and all integration edit leases close, merging supervisor stages and commits fix, proves clean worktree, freezes observed full `HEAD`, records `integration.head_sha` plus commit/freeze evidence, sets existing combined-review gate `accepted` after every finding has clearing disposition, then sets substate `final_verify` in one transition. Failed freeze or unresolved finding leaves combined-review gate failed and routes terminal blocked report before final-verify gate creation
- final verification due: create one `final_verify` gate with `pending` status at frozen candidate SHA before checks; set `integration.final_verify_gate_id` to its ID; retain substate `final_verify`. Blocker discovered after this transition sets existing gate `not_run`; blocker preventing this transition leaves pointer null
...
- accepted final merger `Stage: final`, `Status: blocked`, aggregate `Final validation: not_run`, existing final-verify gate: set gate `not_run`; set validation `not_run`, blocker/evidence, substate `blocked`, and run state `BLOCKED` once; retain every attempted nonnull gate ID/status
- accepted final merger `Stage: final`, `Status: blocked`, aggregate `Final validation: not_run`, blocker before final-verify gate creation: keep `final_verify_gate_id: null`; keep `combined_review_gate_id: null` when combined review never started, otherwise retain its nonnull ID/status; set validation `not_run`, blocker/evidence, substate `blocked`, and run state `BLOCKED` once
...
Final-stage aggregate report references existing combined-review/fix fields; it never reapplies their transitions or writes intermediate `final_verify`. `complete` requires aggregate validation `pass`. `blocked` requires aggregate `fail | not_run` plus blocker; each attempted gate has nonnull ID/status, each unattempted gate has null ID/status. `not_due` is legal only at intermediate stage; no final-stage gate exists. Per-check `pass | fail | not_run` maps to evidence `passed | failed | not_run`; `not_run` requires earlier blocker. Final-stage per-check `not_due` rejects.
...
- Capacity invariant: `requested_slots` and every `reservations[*].slot_count` are positive integers. `0 <= effective_limit <= configured_limit`; `available_count = effective_limit - live_count - reserved_count`; counts nonnegative; `reserved_count` equals held reservation slot sum; `live_count` equals live agent ID count. After reconciliation, reservation/dispatch/spawn is legal only when `1 <= requested_slots <= available_count` and `live_count + reserved_count + requested_slots <= effective_limit <= configured_limit`. Nonpositive request is invalid; zero or insufficient availability queues/waits. Invalid or queued request creates no reservation, dispatch, attempt, lease, or spawn side effect.
...
- Blocked-before-check invariant: final-stage report uses aggregate `not_run`; `integration.validation_status` becomes `not_run`; blocker record targets `verification`; run enters `BLOCKED` once. Blocker before final-verify gate creation leaves that pointer null. Blocker after gate creation sets existing nonnull gate `not_run`. No validation evidence claims execution.
