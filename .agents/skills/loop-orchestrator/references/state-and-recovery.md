...
- Marker/skip/input semantics: [`AGENTS.md`](../../../../AGENTS.md) `Unity execution` production bake gate sole owner. Determine invalidation from source-defined lighting probe after non-lighting assembly. Changed path alone is insufficient: animation clips/controllers, dynamic FBX renderers, dynamic prefabs, or non-lighting scene changes preserve proof unless probe reports lighting-input digest change.
