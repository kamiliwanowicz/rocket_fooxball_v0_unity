...
Invocation contract -> call `Tools/Validation/Invoke-MovementLabWorkflow.ps1` through `Tools/Tests/Invoke-HarnessTests.ps1`; pass `-Mode`, `-PlanOnly`, `-LedgerPath`, and `-EvidenceRoot`. Carry prior accepted `-LedgerPath` across retries and dependent invocations. Harness defect -> run `-PlanOnly` fixture first; no Unity mutation until `harness-unit` passes.

Production bake -> `RocketFooxball.Editor.MovementLabBuilder.BakeMovementLabLighting` owns skip/rebuild from lighting inputs. Exact marker `[MovementLab] production bake skipped: lighting inputs current (digest <64-hex>)` -> `reused`, `bakeCount=0`; absent marker -> one bake, `bakeCount=1`; duplicate/invalid marker -> `blocked`. Generated output hashes never decide.

Run bake budget -> sum `bakeCount` across every run `workflow-result.json` in bound evidence. Total `>2` -> halt `blocked` with file/count evidence; no further Unity.
...
- post-proof fix -> invalidate only rows whose declared paths intersect fix; lighting-input intersection forces builder-gate bake rerun.
