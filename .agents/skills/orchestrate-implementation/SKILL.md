...
Worker validation defaults to fast/local checks. A worker runs development proof only when task ownership names it. Before every workflow or Unity invocation, run `Tools/Tests/Invoke-HarnessTests.ps1` for `harness-unit`; require `<10s`, no Unity process, and no project lock. Guard paths -> `Tools/Tests/**`, `Tools/Validation/*.ps1`, `Assets/_Game/Editor/MovementLab/*.cs`. Red -> `blocked`; repair guard paths, then rerun harness green before any workflow/Unity retry. Before project-mutating production-final Unity proof, orchestrator verifies zero writers, clean exact source SHA, one Unity lease, and accepted checkpoint/fix state. Unity process and project lock checks apply before and after every Unity step.
...
Invocation contract -> after green `harness-unit`, run `Tools/Validation/Invoke-MovementLabWorkflow.ps1 -Mode <...> -ProjectPath <...>` with applicable `-PlanOnly`, `-LedgerPath`, and `-EvidenceRoot`. Carry prior accepted `-LedgerPath` across retries and dependent invocations. Never pass workflow arguments to test runner.
