...
- Stale Unity lock recovery is allowed only after a waited Unity command fails. Immediately before deletion, fail closed unless all of these are proven: zero `Unity.exe`, `UnityShaderCompiler.exe`, or `LightBaker.exe` processes match the same canonical short project path; the target is exactly that canonical project's `Temp\UnityLockfile` or `Library\UnityLockfile`; it is a zero-byte regular non-reparse file within the canonical project; an exclusive open succeeds; and no workflow lease or owner is active. Delete only that exact target with `-LiteralPath` or equivalent, never a glob, recursive command, or broader path. Record the proof and verify and record post-delete absence. A live or ambiguous process/owner, nonzero or reparse target, failed exclusive open, or active/ambiguous lease remains blocking. Unity recreates the sentinel on its next launch. `Remove-LockSentinelExact` and `Wait-UnityRelease` in `Tools/Validation/Invoke-MovementLabWorkflow.ps1` are the hardened implementation reference.
- Unity Test Runner CLI: `-runTests` + `-quit` incompatible under pinned `com.unity.test-framework`. Test command -> omit `-quit`; Test Runner exits Unity after completion. Compile-only command may use `-quit`. Plan/example combining both -> stale ceremony; this rule wins. Omit only `-quit`, flag per Instruction issues.
- Unity test gate: exit code `0` alone insufficient. Requested result XML must exist, parse, report `total > 0`, `failed == 0`, `inconclusive == 0`; missing, empty, or unparseable XML -> fail immediately and block every downstream gate. Then confirm Unity process and project lock release.
...
- Idempotent-operation gate: predicted exact changed-path count/status is expectation, not success predicate. Operation succeeds + semantic contract validates + GUID/pairs/integrity remain valid + comparator covers every actual change + unchanged path was already compliant -> accept validated no-op and continue. Never force byte churn or block solely to match predicted count. Block real semantic failure, integrity/scope failure, or missing authority.
...
- Visual-evidence capture tooling: `Tools/Validation/Capture-BrightArenaVisuals.ps1` + `Assets/_Game/Editor/BrightArenaVisualCapture.cs`. Zero callers is intentional -> retained as template for agent screenshot capture. Never delete as dead code. Reuse blockers before hooking it anywhere: fixed six images (three hardcoded views x High/Low, exact count asserted), `EvidenceRoot` must sit under `C:\wt\`, refuses or waits while an Editor owns the project, and runs the full harness pre-gate first. The wrapper does not automatically clean a stale lock and can strand one after an exception; apply the stale-lock recovery rule above, using `Remove-LockSentinelExact` and `Wait-UnityRelease` in `Tools/Validation/Invoke-MovementLabWorkflow.ps1` as the hardened implementation reference.
