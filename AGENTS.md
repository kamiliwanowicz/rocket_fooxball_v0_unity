...
- Generated-YAML whitespace normalization runs last in every stage that persists generated assets — after the final `SaveAssets`/`ImportAsset`/`Finalize*Persistence` call. Later import/save reintroduces trailing whitespace, so early normalization leaves output bytes non-fixed-point and burns regenerate rounds chasing a stable diff.
...
- Capture pre/post Git status. Reject GUID churn, broken asset/`.meta` pairing, and unrelated reserialization. Accept builder-owned generated YAML reserialization, `fileID`/whitespace changes, and bake nondeterminism; review generated churn semantically and keep it in separate commit `chore: regenerate MovementLab outputs`. Remove only newly generated IDE files. Never judge churn by diff line count -> sole verdict `Tools/Validation/Compare-GeneratedYaml.ps1 -Base <sha> -Head <sha>` (`WORKTREE` ok) -> `SEMANTIC:`/`DANGLING:`, dangling up -> reject; `-Verbose` -> graph detail.

## Unity execution

- Tooling: no Computer Use or related `sky.documentation` / `node_repl` tools.
- Unity worktrees: use short paths such as `C:\wt\<id>`. Existing long path -> verified junction or `subst` drive. Use same short project path for all Unity commands and process checks. Evidence paths obey same limit: create short evidence root (junction such as `C:\wt\<id>e`) before worker dispatch and probe file creation at deepest expected evidence path. Durable evidence stays under Git-common run root, reached through junction. Workflow enforces same rule: `Assert-EvidencePathBudget` rejects over-long evidence path and write-probes deepest path before lease, Unity, or any product mutation.
...
- Harness pre-gate: before any Unity-mutating workflow, run `powershell -NoProfile -ExecutionPolicy Bypass -File Tools/Tests/Invoke-HarnessTests.ps1` harness-unit suite; finish in `<30s` without Unity process or project lock. Failure or timeout blocks Unity.
