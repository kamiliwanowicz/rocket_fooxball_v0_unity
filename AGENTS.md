...
- External stale Unity lock recovery is allowed only after failed waited Unity command has exited. Immediately before deletion, fail closed unless all are proven: every workflow lease is absent; zero `Unity.exe`, `UnityShaderCompiler.exe`, or `LightBaker.exe` processes match same canonical short project path; target is exactly that canonical project's `Temp\UnityLockfile` or `Library\UnityLockfile`; target is zero-byte regular non-reparse file within canonical project; and exclusive `FileShare.None`-style open succeeds. Delete only exact target with `-LiteralPath` or equivalent, never glob, recursive command, or broader path. Record proof, then verify and record post-delete absence. Live or ambiguous process, present or ambiguous lease, nonzero or reparse target, or failed exclusive open remains blocking. Unity recreates sentinel on next launch.
