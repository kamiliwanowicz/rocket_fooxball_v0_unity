...
- Capture pre/post Git status. Classify changed generated output from authoritative inventory + exact task declaration; ownership affects scope only. Every changed authoritative output needs exact comparator coverage: selected path/header, `SEMANTIC:`, `DANGLING:`, `GUID:`, `PAIRS:`, `UNSUPPORTED:`. Builder-traced task-declared semantic changes are allowed. Reject incomplete coverage, unknown types, increased dangling references, existing-GUID churn, broken asset/`.meta` pairs, or undeclared output. Binary hashes bind provenance only; never gate acceptance or nondeterminism. Attach evidence at checkpoint/reviewer even when separate regeneration commit excludes raw diff. Keep generated churn in separate `chore: regenerate MovementLab outputs` commit. Remove only newly generated IDE files.
